...
class LSTMModel(nn.Module):
    def __init__(
        self, input_dim: int, hidden_layer_size: int = 100, output_dim: int = 1, num_layers: int = 1
    ):
        super().__init__()
        self.hidden_layer_size = hidden_layer_size
        self.num_layers = num_layers
        self.lstm = nn.LSTM(input_dim, hidden_layer_size, num_layers=num_layers, batch_first=True, dropout=0.2 if num_layers > 1 else 0)
...
class NNModel(nn.Module):
    def __init__(self, input_dim: int, seq_length: int, output_dim: int = 1):
        super().__init__()
...
class GRUModel(nn.Module):
    def __init__(
        ...
        self.hidden_layer_size = hidden_layer_size
        self.num_layers = num_layers
        self.gru = nn.GRU(input_dim, hidden_layer_size, num_layers=num_layers, batch_first=True, dropout=0.2 if num_layers > 1 else 0)
        self.linear = nn.Linear(hidden_layer_size, output_dim)
